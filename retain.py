...
def dropout_layer(state_before, mode, srng, keep_prob=0.5):
    if keep_prob < 1.0:
        mask = srng.binomial(n=1, p=keep_prob, size=state_before.shape)
        # Cast mask to the correct float type
        mask = T.cast(mask, state_before.dtype)
        return T.switch(T.eq(mode, 1), state_before * mask / keep_prob, state_before)
    else:
        return state_before
...
def gru_layer(tparams, emb, name, hiddenDimSize):
	timesteps = T.cast(emb.shape[0], 'int32')
	if emb.ndim == 3: n_samples = T.cast(emb.shape[1], 'int32')
	else: n_samples = 1
...
def build_model(tparams, options, W_emb=None):
	...
	alphaHiddenDimSize = options['alphaHiddenDimSize']
	betaHiddenDimSize = options['betaHiddenDimSize']
	srng = RandomStream(seed=1234)

	mode = T.scalar('mode', dtype='int32') # 1 for training, 0 for inference
	...
	n_timesteps = T.cast(x.shape[0], 'int32')
	n_samples = T.cast(x.shape[1], 'int32')

	if options['embFineTune']: emb = T.dot(x, tparams['W_emb'])
	else: emb = T.dot(x, W_emb)

	emb = dropout_layer(emb, mode, srng, keep_prob_emb)

	if useTime: temb = T.concatenate([emb, t.reshape([n_timesteps,n_samples,1])], axis=2) #Adding the time element to the embedding
	else: temb = emb
	
	def attentionStep(att_timesteps, temb,
					  W_gru_a, U_gru_a, b_gru_a,
					  W_gru_b, U_gru_b, b_gru_b,
					  w_alpha, b_alpha, W_beta, b_beta):
		_tparams = {
			'W_gru_a': W_gru_a, 'U_gru_a': U_gru_a, 'b_gru_a': b_gru_a,
			'W_gru_b': W_gru_b, 'U_gru_b': U_gru_b, 'b_gru_b': b_gru_b,
			'w_alpha': w_alpha, 'b_alpha': b_alpha,
			'W_beta': W_beta, 'b_beta': b_beta
		}
		reverse_emb_t = temb[:att_timesteps][::-1]
		reverse_h_a = gru_layer(_tparams, reverse_emb_t, 'a', alphaHiddenDimSize)[::-1] * 0.5
		reverse_h_b = gru_layer(_tparams, reverse_emb_t, 'b', betaHiddenDimSize)[::-1] * 0.5

		if useTime:
			emb_for_c = temb[:, :, :embDimSize]
		else:
			emb_for_c = temb

		preAlpha = T.dot(reverse_h_a, _tparams['w_alpha']) + _tparams['b_alpha']
		preAlpha = preAlpha.reshape((preAlpha.shape[0], preAlpha.shape[1]))
		alpha = (special.softmax(preAlpha.T)).T

		beta = T.tanh(T.dot(reverse_h_b, _tparams['W_beta']) + _tparams['b_beta'])
		c_t = (alpha[:,:,None] * beta * emb_for_c[:att_timesteps]).sum(axis=0)
		return c_t

	counts = T.arange(n_timesteps)+ 1
	non_seqs = [temb,
				tparams['W_gru_a'], tparams['U_gru_a'], tparams['b_gru_a'],
				tparams['W_gru_b'], tparams['U_gru_b'], tparams['b_gru_b'],
				tparams['w_alpha'], tparams['b_alpha'], tparams['W_beta'], tparams['b_beta']]
	c_t, updates = pytensor.scan(fn=attentionStep,
							   sequences=[counts],
							   non_sequences=non_seqs,
							   outputs_info=None,
							   name='attention_layer',
							   n_steps=n_timesteps)
	c_t = dropout_layer(c_t, mode, srng, keep_prob_context)

	preY = T.sigmoid(T.dot(c_t, tparams['w_output']) + tparams['b_output'])
	...
	if useTime: return mode, x, y, t, lengths, cost_noreg, cost, y_hat
	else: return mode, x, y, lengths, cost_noreg, cost, y_hat

def adadelta(tparams, grads, x, y, lengths, cost, options, mode, t=None):
	zipped_grads = [pytensor.shared(p.get_value() * numpy_floatX(0.), name='%s_grad' % k) for k, p in tparams.items()]
	running_up2 = [pytensor.shared(p.get_value() * numpy_floatX(0.), name='%s_rup2' % k) for k, p in tparams.items()]
	running_grads2 = [pytensor.shared(p.get_value() * numpy_floatX(0.), name='%s_rgrad2' % k) for k, p in tparams.items()]

	zgup = [(zg, g) for zg, g in zip(zipped_grads, grads)]
	rg2up = [(rg2, 0.95 * rg2 + 0.05 * (g ** 2)) for rg2, g in zip(running_grads2, grads)]

	if len(options['timeFile']) > 0:
		f_grad_shared = pytensor.function([x, y, t, lengths], cost, updates=zgup + rg2up, name='adadelta_f_grad_shared', givens={mode: np.int32(1)})
	else:
		f_grad_shared = pytensor.function([x, y, lengths], cost, updates=zgup + rg2up, name='adadelta_f_grad_shared', givens={mode: np.int32(1)})

	updir = [-T.sqrt(ru2 + 1e-6) / T.sqrt(rg2 + 1e-6) * zg for zg, ru2, rg2 in zip(zipped_grads, running_up2, running_grads2)]
	ru2up = [(ru2, 0.95 * ru2 + 0.05 * (ud ** 2)) for ru2, ud in zip(running_up2, updir)]
	param_up = [(p, p + ud) for p, ud in zip(tparams.values(), updir)]

	f_update = pytensor.function([], [], updates=ru2up + param_up, on_unused_input='ignore', name='adadelta_f_update')
...
def adam(cost, tparams, lr=0.0002, b1=0.1, b2=0.001, e=1e-8):
	...
	i = pytensor.shared(numpy_floatX(0.))
	i_t = i + 1.
	fix1 = 1. - (1. - b1)**i_t
	fix2 = 1. - (1. - b2)**i_t
	lr_t = lr * (T.sqrt(fix2) / fix1)
	for p, g in zip(tparams.values(), grads):
		m = pytensor.shared(p.get_value() * 0.)
		v = pytensor.shared(p.get_value() * 0.)
		m_t = (b1 * g) + ((1. - b1) * m)
...
def calculate_auc(test_model, dataset, options):
	...
	for index in range(n_batches):
		...
		if useTime:
			...
			scores = test_model(x, t, lengths, np.int32(0))
		else:
			x, lengths = padMatrixWithoutTime(batchX, options)
			scores = test_model(x, lengths, np.int32(0))
...
def calculate_cost(test_model, dataset, options):
	...
	for index in range(n_batches):
		...
		if useTime:
			...
			scores = test_model(x, y, t, lengths, np.int32(0))
		else:
			x, lengths = padMatrixWithoutTime(batchX, options)
			y = np.array(dataset[1][index*batchSize:(index+1)*batchSize]).astype(config.floatX)
			scores = test_model(x, y, lengths, np.int32(0))
...
def train_RETAIN(
	...
	if useTime and embFineTune:
		...
		mode, x, y, t, lengths, cost_noreg, cost, y_hat =  build_model(tparams, options)
		if solver=='adadelta':
			grads = T.grad(cost, wrt=list(tparams.values()))
			f_grad_shared, f_update = adadelta(tparams, grads, x, y, lengths, cost, options, mode, t)
		elif solver=='adam':
			updates = adam(cost, tparams)
			update_model = pytensor.function(inputs=[x, y, t, lengths, mode], outputs=cost, updates=updates, name='update_model')
		get_prediction = pytensor.function(inputs=[x, t, lengths, mode], outputs=y_hat, name='get_prediction')
		get_cost = pytensor.function(inputs=[x, y, t, lengths, mode], outputs=cost_noreg, name='get_cost')
	elif useTime and not embFineTune:
		print('using time information, not fine-tuning code representations')
		W_emb = pytensor.shared(params['W_emb'], name='W_emb')
		mode, x, y, t, lengths, cost_noreg, cost, y_hat =  build_model(tparams, options, W_emb)
		if solver=='adadelta':
			grads = T.grad(cost, wrt=list(tparams.values()))
			f_grad_shared, f_update = adadelta(tparams, grads, x, y, lengths, cost, options, mode, t)
		elif solver=='adam':
		...
		mode, x, y, lengths, cost_noreg, cost, y_hat =  build_model(tparams, options)
		if solver=='adadelta':
			grads = T.grad(cost, wrt=list(tparams.values()))
			f_grad_shared, f_update = adadelta(tparams, grads, x, y, lengths, cost, options, mode)
		elif solver=='adam':
			updates = adam(cost, tparams)
			update_model = pytensor.function(inputs=[x, y, lengths, mode], outputs=cost, updates=updates, name='update_model')
		get_prediction = pytensor.function(inputs=[x, lengths, mode], outputs=y_hat, name='get_prediction')
		get_cost = pytensor.function(inputs=[x, y, lengths, mode], outputs=cost_noreg, name='get_cost')
	elif not useTime and not embFineTune:
		print('not using time information, not fine-tuning code representations')
		W_emb = pytensor.shared(params['W_emb'], name='W_emb')
		mode, x, y, lengths, cost_noreg, cost, y_hat =  build_model(tparams, options, W_emb)
		if solver=='adadelta':
			grads = T.grad(cost, wrt=list(tparams.values()))
			f_grad_shared, f_update = adadelta(tparams, grads, x, y, lengths, cost, options, mode)
		elif solver=='adam':
		...
		for index in random.sample(range(n_batches), n_batches):
			...
			if useTime:
				...
				if solver=='adadelta':
					...
					costValue = update_model(x, y, t, lengths, np.int32(1))
			else:
				x, lengths = padMatrixWithoutTime(batchX, options)
				if solver=='adadelta':
					...
					costValue = update_model(x, y, lengths, np.int32(1))
