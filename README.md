...
1. Install [python](https://www.python.org/), [PyTorch](https://pytorch.org/). We use Python 3.12, PyTorch 2.5.0.
...
The command is `python3 retain.py <3digitICD9.seqs file> 942 <morts file> <output path> --simple_load --n_epochs 100 --keep_prob_context 0.8 --keep_prob_emb 0.5`.
...
1. RETAIN's training dataset needs to be a Python pickled list of list of list. The outermost list corresponds to patients, the intermediate to the visit sequence each patient made, and the innermost to the medical codes (e.g. diagnosis codes, medication codes, procedure codes, etc.) that occurred within each visit.
...
This list of list of list needs to be pickled using pickle. We will refer to this file as the "visit file".
...
3. The label dataset (let us call this "label file") needs to be a Python pickled list. Each element corresponds to the true label of each patient. For example, 1 can be the case patient and 0 can be the control patient. If there are two patients where only the first patient is a case, then we should have [1,0].
...
"time file" needs to be prepared as a Python pickled list of list. The outermost list corresponds to patients, and the innermost to the time information of each visit.
...
`python3 retain.py <visit file> <# codes in the visit file> <label file> <output path>`  
...
6. Additional options can be specified such as the size of the batch size, the number of epochs, etc. Detailed information can be accessed by `python3 retain.py --help`
...
RETAIN checks the AUC of the validation set after each epoch, and if it is higher than all previous values, it will save the current model. The model file is a PyTorch model file.

**Step 6: Testing your model**

1. Using the file "test_retain.py", you can calculate the contributions of each medical code at each visit. First you need to have a trained model that was saved by PyTorch. Note that you need to know the configuration with which you trained RETAIN (e.g. use of `--time_file`, use of `--use_log_time`.)
...
This file (let's call this "mapping file") need to be a Python pickled dictionary where the keys are the string medical codes and the values are the corresponding intergers. 
...
`python3 test_retain.py <model file> <visit file> <label file> <mapping file> <output file>`
